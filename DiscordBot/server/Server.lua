...
-- System Infos
-- PerformHttpRequest(DiscordWebhookSystemInfos, function(Error, Content, Head) end, 'POST', json.encode({username = SystemName, content = '**SERVER UP!! HAVE FUN WITH INDO PUBLIC ROLEPLAY**'}), { ['Content-Type'] = 'application/json' })

AddEventHandler('playerConnecting', function()
	local steamhex = GetPlayerIdentifier(source)
	TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookSystemInfos, SystemName,
		'```css\n+ ' .. GetPlayerName(source) .. ' [' .. steamhex .. '] connecting\n```', SystemAvatar, false)
end)

AddEventHandler('playerDropped', function(Reason)
	local steamhex = GetPlayerIdentifier(source)
	TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookSystemInfos, SystemName,
		'```fix\n- ' .. GetPlayerName(source) .. ' [' .. steamhex .. '] left (' .. Reason .. ')\n```', SystemAvatar, false)
end)

-- Killing Log
RegisterServerEvent('DiscordBot:playerMati')
AddEventHandler('DiscordBot:playerMati', function(Message, Weapon)
	local steamhex = GetPlayerIdentifier(source)
	...
	TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookKillinglogs, SystemName,
		Message ..
		' `' ..
...
AddEventHandler('chatMessage', function(Source, Name, Message)
	...
	if not IsCommand(MessageSplitted, 'Blacklisted') then
		...
		if GetIDFromSource('steam', Source) then
			PerformHttpRequest('http://steamcommunity.com/profiles/' ..
				tonumber(GetIDFromSource('steam', Source), 16) .. '/?xml=1', function(Error, Content, Head)
				local SteamProfileSplitted = stringsplit(Content, '\n')
				for i, Line in ipairs(SteamProfileSplitted) do
					if Line:find('<avatarFull>') then
						AvatarURL = Line:gsub('	<avatarFull><!%[CDATA%[', ''):gsub(']]></avatarFull>', '')
						TriggerEvent('DiscordBot:ToDiscord', Webhook, Name .. ' [ID: ' .. Source .. ']', Message, AvatarURL, false, Source
							, TTS) --Sending the message to discord
						break
					end
				end
			end)
...
AddEventHandler('DiscordBot:ToDiscord', function(WebHook, Name, Message, Image, External, Source, TTS)
				...
				PerformHttpRequest('http://steamcommunity.com/profiles/' ..
					...
					for i, Line in ipairs(SteamProfileSplitted) do
						if Line:find('<avatarFull>') then
							...
							return PerformHttpRequest(WebHook, function(Error, Content, Head) end, 'POST',
								json.encode({ username = Name, content = Message, avatar_url = Image, tts = TTS }),
	...
	PerformHttpRequest(WebHook, function(Error, Content, Head) end, 'POST',
		json.encode({ username = Name, content = Message, avatar_url = Image, tts = TTS }),
		{ ['Content-Type'] = 'application/json' })
end)
