...
function startExercise(animInfo, pos)
    ...
    TaskPlayAnim(playerPed, animInfo["enterDict"], animInfo["enterAnim"], 8.0, -8.0, animInfo["enterTime"], 0, 0.0, 0, 0
        , 0)
        ...
        while canExercise do
            ...
            if procent <= 99 then
                ...
                if IsControlJustPressed(0, Keys["SPACE"]) then -- press space to train
                    canExercise = false
                    TaskPlayAnim(playerPed, animInfo["actionDict"], animInfo["actionAnim"], 8.0, -8.0,
                        animInfo["actionTime"], 0, 0.0, 0, 0, 0)
                ...
                ExitTraining(animInfo["exitDict"], animInfo["exitAnim"], animInfo["exitTime"])
                -- exports['stress_utk']:RemoveStress('slow', 100000, 5)
                exports['stress_utk']:RemoveStress('instant', 50000) -- Removes 100.000 (%10) stress instantly
                -- Here u can put a event to update some sort of skill or something.
                -- this is when u finished your exercise
            end
            -- exports['stress_utk']:RemoveStress('slow', 10000, 5)
        end
    end)
end
...
RegisterCommand("motion", function()
    ...
    Citizen.CreateThread(function()
        while doingMotion do
            ...
            DrawText2D(0.505, 0.95, 1.0, 1.0, 0.4,
                "~b~Percentage:~w~ " .. tonumber(string.format("%.1f", motionProcent / 1000)) .. "%", 255, 255, 255, 255)
