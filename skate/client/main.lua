...
RCCar.Start = function()
	...
	while DoesEntityExist(RCCar.Entity) and DoesEntityExist(RCCar.Driver) do
		...
		if distanceCheck <= ConfigSkate.LoseConnectionDistance then
...
RCCar.HandleKeys = function(distanceCheck)
	...
	if distanceCheck < ConfigSkate.LoseConnectionDistance then
		local overSpeed = (GetEntitySpeed(RCCar.Entity) * 3.6) > ConfigSkate.MaxSpeedKmh
		...
		if IsControlPressed(0, 22) and Attached then
			...
			if not IsEntityInAir(RCCar.Entity) then
				...
				boost = ConfigSkate.MaxJumpHeigh * duration / 250.0
				if boost > ConfigSkate.MaxJumpHeigh then boost = ConfigSkate.MaxJumpHeigh end
...
RCCar.Spawn = function()
	-- models to load
	RCCar.LoadModels({ GetHashKey('bmx'), 68070371, GetHashKey('p_defilied_ragdoll_01_s'), 'pickup_object',
		'move_strafe@stealth', 'move_crouch_proto' })
	...
	AttachEntityToEntity(RCCar.Skate, RCCar.Entity, GetPedBoneIndex(PlayerPedId(), 28422), 0.0, 0.0, -0.40, 0.0, 0.0, 90.0,
...
RCCar.Attach = function(param)
	...
	if param == 'place' then
		-- Place longboard
		AttachEntityToEntity(RCCar.Entity, PlayerPedId(), GetPedBoneIndex(PlayerPedId(), 28422), -0.1, 0.0, -0.2, 70.0, 0.0,
			270.0, 1, 1, 0, 0, 2, 1)
