...
function TimeToString(time)
	local formattedDate = table.concat(ConfigCooldowns.DateFormat, '/') .. " %H:%M:%S"
	local str = os.date(formattedDate, time)

	if ConfigCooldowns.Debug then
		print(string.format('^3[qb-cooldowns]^7 TimeToString(%s) formattedDate: %s; final string: %s', time,
		formattedDate, str))
	end
	...
	if Cooldowns[type] then
		...
		exports.oxmysql:fetch('SELECT `cooldown` FROM `cooldowns` WHERE `type` = @type', {
			...
			if cooldowns ~= nil and cooldowns[1] ~= nil then
				...
				cb(ConfigCooldowns.InexistentCooldownStartsAt0 and 0 or os.time())
			end
		end)
	end
end
	...
	DoesCooldownExist(function(doesExist)
		if doesExist then
			...
			exports.oxmysql:execute('INSERT INTO `cooldowns` (`type`, `cooldown`) VALUES (@type, @cooldown)', {
				...
				if ConfigCooldowns.Debug then
					print(string.format(
					'^3[qb-cooldowns]^7 The cooldown %s has been established to %s, creating a new entry with ID %s',
					type, cooldown, id))
				end
			end)
		end
...
if ConfigCooldowns.RegisterSetCooldown then
...
ESX.RegisterServerCallback('qb-cooldowns:getCooldown', function(source, cb, type, format)
	if ConfigCooldowns.Debug then
		print(string.format('^3[qb-cooldowns]^7 getCooldown qb callback has been called by source %s',
		source or 'resource'))
	end
...
ESX.RegisterServerCallback('qb-cooldowns:getTime', function(source, cb, type, format)
	if ConfigCooldowns.Debug then
