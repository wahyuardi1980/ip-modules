...
Citizen.CreateThread(function()
    ...
    if ConfigGps.useRflxMulti then
    ...
    if ConfigGps.blipGroup.renameGroup then
        AddTextEntryByHash(`BLIP_OTHPLYR`, ConfigGps.blipGroup.groupName .. '~w~')
    end
    checkJob()
end)
...
AddEventHandler('onResourceStop', function(resourceName)
    ...
    if ConfigGps.bigmapTags then
        DisplayPlayerNameTagsOnBlips(false)
    end
    removeAllBlips()
end)

if ConfigGps.useBaseEvents then
    AddEventHandler("baseevents:enteredVehicle", function(veh, seat, vehiclelabel)
        inVeh = true
        if onDuty then
            inVehChecks(veh, seat, vehiclelabel)

            local cfg = ConfigGps.emergencyJobs[PlayerData.job.name].vehBlip and
                ConfigGps.emergencyJobs[PlayerData.job.name].vehBlip[GetEntityModel(veh)] or nil
...
function checkJob()
    if PlayerData and PlayerData.job and ConfigGps.emergencyJobs[PlayerData.job.name] and
        ConfigGps.emergencyJobs[PlayerData.job.name].ignoreDuty then
        goOnDuty()
    end
end
...
function goOnDuty()
    ...
    if ConfigGps.notifications.enable and ConfigGps.notifications.useMythic then
        exports['mythic_notify']:DoHudText('inform', ConfigGps.notifications.onDutyText)
    elseif ConfigGps.notifications.enable then
        ESX.ShowNotification(ConfigGps.notifications.onDutyText)
    end
    ...
    if inVeh then
        TriggerServerEvent('rflx_pdblips:enteredVeh',
            ConfigGps.emergencyJobs[PlayerData.job.name].vehBlip[GetEntityModel(GetVehiclePedIsIn(PlayerPedId(), false))
            ])
    end
end
...
function goOffDuty()
    ...
    if ConfigGps.notifications.enable and ConfigGps.notifications.useMythic then
        exports['mythic_notify']:DoHudText('inform', ConfigGps.notifications.offDutyText)
    elseif ConfigGps.notifications.enable then
        ESX.ShowNotification(ConfigGps.notifications.offDutyText)
    end
...
AddEventHandler('rflx_pdblips:receiveData', function(myId, data) -- ugly ass event
    for k, v in pairs(data) do
        ...
        local canSee = ConfigGps.emergencyJobs[v.job].canSee and
            ConfigGps.emergencyJobs[v.job].canSee[PlayerData.job.name]
            ...
            if myId ~= v.playerId then
                if cId ~= -1 then
                    ...
                    elseif nearBlips[v.playerId].siren then -- blip color flash
                        nearBlips[v.playerId].sirenState = v.flashColors[nearBlips[v.playerId].sirenState + 1] and
                            nearBlips[v.playerId].sirenState + 1 or 1
                    ...
                    elseif longBlips[v.playerId].siren then -- blip color flash
                        longBlips[v.playerId].sirenState = v.flashColors[longBlips[v.playerId].sirenState + 1] and
                            longBlips[v.playerId].sirenState + 1 or 1
                        updateBlipFlash(longBlips[v.playerId].blip, v.flashColors[longBlips[v.playerId].sirenState])
                    end
                end
            elseif ConfigGps.selfBlip then
                ...
                AddTextComponentString(ConfigGps.usePrefix and v.prefix .. ' ' .. v.name or v.name)
...
function setupBlip(blip, data)
    SetBlipSprite(blip, ConfigGps.emergencyJobs[data.job].blip.sprite)
    SetBlipDisplay(blip, 2)
    SetBlipScale(blip, ConfigGps.emergencyJobs[data.job].blip.scale or 0.9)
    SetBlipColour(blip, ConfigGps.emergencyJobs[data.job].blip.color)
    SetBlipFlashes(blip, false)
    SetBlipShowCone(blip, ConfigGps.blipCone)
    SetBlipCategory(blip, 7)
    SetBlipShowCone(blip, true)
    BeginTextCommandSetBlipName("STRING")
    if ConfigGps.font.useCustom then
        AddTextComponentString("<font face='" ..
        ConfigGps.font.name .. "'>" .. data.prefix .. " " .. data.name .. "</font>")
    else
        AddTextComponentString(ConfigGps.usePrefix and data.prefix .. ' ' .. data.name or data.name)
    end
...
function vehBlipSetup(blip, data)
    if data.inVeh then
        ...
        SetBlipDisplay(blip, 2)
        SetBlipScale(blip, ConfigGps.emergencyJobs[data.job].blip.scale or 1.0)
        SetBlipColour(blip, data.vehColor)
        SetBlipShowCone(blip, ConfigGps.blipCone)
        BeginTextCommandSetBlipName("STRING")
        if ConfigGps.font.useCustom then
            AddTextComponentString("<font face='" .. ConfigGps.font.name .. "'>" .. data.prefix .. " " ..
            data.name .. "</font>")
        else
            AddTextComponentString(ConfigGps.usePrefix and data.prefix .. ' ' .. data.name or data.name)
        end
        ...
        SetBlipSprite(blip, ConfigGps.emergencyJobs[data.job].blip.sprite)
        SetBlipDisplay(blip, 2)
        SetBlipScale(blip, ConfigGps.emergencyJobs[data.job].blip.scale or 0.7)
        SetBlipColour(blip, ConfigGps.emergencyJobs[data.job].blip.color)
        SetBlipShowCone(blip, ConfigGps.blipCone)
        BeginTextCommandSetBlipName("STRING")
        if ConfigGps.font.useCustom then
