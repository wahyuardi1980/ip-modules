...
CreateThread(function()
    ...
    framework = GetResourceState('es_extended') == 'started' and 'esx' or
        GetResourceState('qb-core') == 'started' and 'qb' or nil
...
AddEventHandler('wasabi_elevator:goToFloor', function(data)
    ...
    local coords = ConfigElevator.Elevators[elevator][floor].coords
    local heading = ConfigElevator.Elevators[elevator][floor].heading
    local ped = cache.ped
...
AddEventHandler('wasabi_elevator:openMenu', function(data)
    ...
    local elevatorData = ConfigElevator.Elevators[elevator]
    local Options = {}

    for k, v in pairs(elevatorData) do
        if k == floor then
            table.insert(Options, {
                title = v.title .. ' (Saat Ini)',
                disabled = true,
...
CreateThread(function()
    for k, v in pairs(ConfigElevator.Elevators) do
        for a, b in pairs(ConfigElevator.Elevators[k]) do
            if b.groups then
