...
AddEventHandler('esx_kekke_tackle:getTackled', function(target)
	...
	AttachEntityToEntity(GetPlayerPed(-1), targetPed, 11816, 0.25, 0.5, 0.0, 0.5, 0.5, 180.0, false, false, false, false, 2
		, false)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(0, Keys['LEFTSHIFT']) and IsControlPressed(0, Keys['E']) and not isTackling then
			...
			if distance ~= -1 and distance <= ConfigTakel.TackleDistance and not isTackling and not isGettingTackled and
				not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				isTackling = true
