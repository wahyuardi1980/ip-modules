...
function UnJail()
	...
	ESX.ShowNotification("Kamu telah keluar dari penjara")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		while jailTime > 0 do
			...
			local Packages = ConfigJail.PrisonWork["Packages"]
...
function PackPackage(packageId)
	local Package = ConfigJail.PrisonWork["Packages"][packageId]
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			local DeliverPosition = ConfigJail.PrisonWork["DeliverPackage"]
