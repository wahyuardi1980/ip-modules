...
CreateThread(function()
	local blip = AddBlipForCoord(ConfigSky.Npc)
	SetBlipSprite(blip, 94)
	SetBlipColour(blip, 2)
	SetBlipScale(blip, 0.65)
	SetBlipAsShortRange(blip, true)
	BeginTextCommandSetBlipName("STRING")
	AddTextComponentString(ConfigSky.Blipname)
	...
	CreateThread(function()
		while waitingTakeOff do
			if GetGameTimer() < gameTimer + tonumber(1000 * Time) then
				...
				drawTxt("TIME UNTIL TAKE OFF: " .. math.ceil(Time - secondsLeft / 1000), 4, 0.5, 0.93, 0.50, 255, 255,
					255, 180)
			end
			Wait(0)
		end
...
function AirfieldPilot()
	if not DoesEntityExist(igpilot) then
		...
		igpilot = CreatePed(4, "ig_pilot", vector3(ConfigSky.Npc), ConfigSky.Heading, false, false)
		...
		exports['qtarget']:AddTargetEntity(igpilot, {
			options = {
				{
					type = "server",
					event = "randol_skydive:server:payforgroup",
					icon = "fa-solid fa-user-group",
					label = "Group Skydive Rp." .. ConfigSky.GroupFee,
				},
				{
					type = "client",
				...
				},
				{
					...
					label = "Solo Jump Rp." .. ConfigSky.SoloFee,
				}
			},
			distance = 2.5,
		})
...
AddEventHandler('randol_skydive:client:skydivetime', function()
	...
	if HasModelLoaded(planeHK) and HasModelLoaded(pilotHK) then
		...
		exports[ConfigSky.FuelExport]:SetFuel(Skydive, 100.0)
		...
		Wait(3000)
		DeletePed(pilot)
		ESX.Game.DeleteVehicle(Skydive)
	end
end)
...
AddEventHandler('randol_skydive:client:skydivesolo', function()
	...
	if HasModelLoaded(planeHK) and HasModelLoaded(pilotHK) then
		Skydive = CreateVehicle(planeHK, ConfigSky.Airplane, ConfigSky.AirplaneHeading, true, false)
		SetPedIntoVehicle(Ped, Skydive, 2)
		GiveWeaponToPed(Ped, GetHashKey("GADGET_PARACHUTE"), true)
...
AddEventHandler('randol_skydive:client:ridewithbuddy', function()
	...
	if foundVehicle == nil then
		-- ESX.Show("Nobody has paid for the Group flight yet!", 'error')
		ESX.ShowNotification("Nobody has paid for the Group flight yet!")
		return
	end
