...
local Queue = {}
-- EDIT THESE IN SERVER.CFG + OTHER OPTIONS IN ConfigQueue.LUA
...
_Queue.JoinCbs = {}
_Queue.TempPriority = {}
_Queue.JoinDelay = GetGameTimer() + ConfigQueue.JoinDelay and ConfigQueue.JoinDelay or 0
...
for id, power in pairs(ConfigQueue.Priority) do
...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s] was prioritized and placed %d/%d in queue", tmp.name, ids[1], _pos
                    , queueCount))
                break
            end
        end
...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function remove()
        ...
        done(ConfigQueue.Language.connectingerr)
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    for i = ConfigQueue.AntiSpamTimer, 0, -1 do
        deferrals.update(ConfigQueue.PleaseWait_1 .. i .. ConfigQueue.PleaseWait_2)
        Citizen.Wait(1000)
    end
    ...
    Citizen.CreateThread(function()
        while connecting do
            ...
            deferrals.update(ConfigQueue.Language.connecting)
        end
    end)
    ...
    local function done(msg, _deferrals)
        ...
        if not msg then
            deferrals.done()
            if ConfigQueue.EnableGrace then Queue:AddPriority(ids[1], ConfigQueue.GracePower, ConfigQueue.GraceTime) end
        else
    ...
    if not ids then
        -- prevent joining
        done(ConfigQueue.Language.iderr)
        CancelEvent()
        Queue:DebugPrint("Dropped " .. name .. ", couldn't retrieve any of their id's")
        return
    end

    if ConfigQueue.RequireSteam and not Queue:IsSteamRunning(src) then
        -- prevent joining
        done(ConfigQueue.Language.steam)
        CancelEvent()
        return
    end
    ...
    if ConfigQueue.PriorityOnly and not Queue:IsPriority(ids) then done(ConfigQueue.Language.wlonly) return end
    ...
    if not pos or not data then
        done(ConfigQueue.Language.err .. " [1]")
    ...
    update(string_format(ConfigQueue.Language.pos ..
        ((Queue:TempSize() and ConfigQueue.ShowTemp) and " (" .. Queue:TempSize() .. " temp)" or "00:00:00"), pos,
        Queue:GetSize(), ""))
    ...
    while true do
        ...
        if not data or not data.deferrals or not data.source or not pos then
            ...
            Queue:DebugPrint(tostring(name ..
                "[" .. ids[1] .. "] was removed from the queue because they had invalid data"))
            return
        end
        ...
        if data.timeout >= ConfigQueue.QueueTimeOut and os_time() - connectTime > 5 then
        ...
        if pos <= 1 and Queue:NotFull() and _Queue.JoinDelay <= GetGameTimer() then
            ...
            update(ConfigQueue.Language.joining, data.deferrals)
            Citizen.Wait(500)
        ...
        local qTime = string_format("%02d", math_floor((seconds % 86400) / 3600)) ..
            ":" ..
            string_format("%02d", math_floor((seconds % 3600) / 60)) ..
            ":" .. string_format("%02d", math_floor(seconds % 60))

        local msg = string_format(ConfigQueue.Language.pos ..
            ((Queue:TempSize() and ConfigQueue.ShowTemp) and " (" .. Queue:TempSize() .. " temp)" or ""), pos,
            Queue:GetSize(), qTime)
        update(msg, data.deferrals)
    end
end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and not endPoint) or data.timeout >= ConfigQueue.ConnectTimeOut) and
                data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                remove(data)
                Queue:DebugPrint(data.name ..
                    "[" .. data.ids[1] .. "] was removed from the connecting queue because they timed out")
            else
                i = i + 1
            end
        end
...
AddEventHandler("onResourceStop", function(resource)
    if Queue.DisplayQueue and Queue.InitHostName and resource == GetCurrentResourceName() then SetConvar("sv_hostname",
        Queue.InitHostName) end
...
if ConfigQueue.DisableHardCap then
...
commands.printq = function()
    ...
    for pos, data in ipairs(Queue:GetQueueList()) do
        Queue:DebugPrint(pos ..
            ...
            " | Last Msg: " ..
            (data.source ~= "debug" and GetPlayerLastMsg(data.source) or "debug") ..
            " | Timeout: " .. data.timeout .. " | Queue Time: " .. data.queuetime() .. " Seconds")
    end
end
...
commands.printc = function()
    ...
    for pos, data in ipairs(Queue:GetConnectingList()) do
        Queue:DebugPrint(pos ..
            ...
            (tostring(data.priority and data.priority or false)) ..
            " | Last Msg: " ..
            (data.source ~= "debug" and GetPlayerLastMsg(data.source) or "debug") .. " | Timeout: " .. data.timeout)
    end
end
...
commands.printtp = function()
    ...
    for k, data in pairs(Queue:GetTempPriorityList()) do
        Queue:DebugPrint(k ..
            ": Power: " ..
            tostring(data.power) .. " | EndTime: " .. tostring(data.endTime) .. " | CurTime: " .. tostring(os_time()))
    end
end
